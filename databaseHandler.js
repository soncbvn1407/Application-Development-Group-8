...
async function countStatistical() {
    let result = {};
    const dbo = await getdbo();
    const orderCounts = await dbo.collection("Order").countDocuments();
    result.orderCounts = orderCounts;

    const feedbackCounts = await dbo.collection("Feedback").countDocuments();
    result.feedbackCounts = feedbackCounts;

    const customerCounts = await dbo.collection("Users").countDocuments();
    result.customerCounts = customerCounts;

    const bookCounts = await dbo.collection("Book").countDocuments();
...
async function findFeedbackById(id) {
    const dbo = await getdbo();
    const feedback = await dbo.collection("Feedback").findOne({ _id: ObjectId(id) })
    return feedback;
}

async function updateRepply(id, content) {
    const dbo = await getdbo();
    const feedback = await dbo.collection("Feedback").updateOne({ "_id": id }, { $set: content }, { "upsert": true })
}
