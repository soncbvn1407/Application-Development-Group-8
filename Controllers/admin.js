...
router.get('/manageCustomer', async(req, res) => {
    ...
    res.render('adminPage', { Customer: arr, statistical: statisticalResult }) //truyền vào property Customer với values =arr
...
router.get("/feedbackManage", async(req, res) => {
    ...
    const statisticalResult = await dbHandler.countStatistical();
    res.render('adminPage', { feedback: result, user: req.session.user, statistical: statisticalResult }) //truyền vào
});
router.post("/feedbackManage/reply", async(req, res) => {
    console.log(req.query)
    const feedback = await dbHandler.findFeedbackById(req.query.id);
    console.log(feedback);
    if (feedback == undefined || feedback == null) {
        //error on input data
    } else if (feedback.reply != null && feedback != undefined) {
        //user update before
    } else {
        feedback.reply = req.query.reply;
        await dbHandler.updateRepply(feedback._id, feedback);
    }
